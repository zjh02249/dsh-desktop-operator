...
const Config = z.object({
	...
	actionLockTimeoutMs: z.number().step(1).min(1).max(12e4).default(5e3),
	actionJournalPath: z.string().default(""),
	actionJournalRetentionDays: z.number().step(1).min(1).max(3650).default(30),
	actionJournalMaxEvents: z.number().step(1).min(100).max(1e5).default(4096),
...
function resolveRuntimeEnv(config) {
	...
	return {
		...
		OPEN_COMPUTER_USE_WINDOWS_ACTION_LOCK_TIMEOUT_MS: String(config.actionLockTimeoutMs ?? 5e3),
		OPEN_COMPUTER_USE_WINDOWS_ACTION_JOURNAL_RETENTION_DAYS: String(config.actionJournalRetentionDays ?? 30),
		OPEN_COMPUTER_USE_WINDOWS_ACTION_JOURNAL_MAX_EVENTS: String(config.actionJournalMaxEvents ?? 4096),
...
function ensureComputerUseActionIdentity(exec) {
	...
	if (typeof args.idempotency_key !== "string" || args.idempotency_key.trim() === "") args.idempotency_key = actionID;
}
...
async function apply(ctx, config) {
	...
	const actionLockTimeoutMs = config.actionLockTimeoutMs ?? 5e3;
	const actionJournalPath = config.actionJournalPath ?? "";
	const actionJournalRetentionDays = config.actionJournalRetentionDays ?? 30;
	const actionJournalMaxEvents = config.actionJournalMaxEvents ?? 4096;
